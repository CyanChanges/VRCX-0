...
fn normalize_vrchat_launch_root(path: &str) -> Result<PathBuf, AppError> {
    let path = PathBuf::from(path.trim());
    if path.as_os_str().is_empty() {
        return Err(AppError::Custom("VRChat launch path is empty.".into()));
    }
    let root = if path
        .file_name()
        ...
        .is_some_and(|name| name.eq_ignore_ascii_case("launch.exe") || name == "steam.sh")
    {
        path.parent()
            ...
            .ok_or_else(|| AppError::Custom("VRChat launch path has no parent.".into()))?
    } else {
        path
    };
    root.canonicalize()
        .map_err(|error| AppError::Custom(format!("VRChat launch path is not accessible: {error}")))
}
