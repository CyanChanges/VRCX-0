...
import {
    ...
    formatClock
} from '../instance-activity/instanceActivityChart';
...
export function ChartEmptyState({ title, description }: any) {
    return (
        <AppEmptyState
            className="min-h-80"
            title={title}
...
export function InstanceActivityDetailChart({
    ...
    const setDetailChartElementRef = useCallback((node: any) => {
    ...
    function openPreviousInstanceInfo() {
        ...
        const startMs = Number.isFinite(firstEntry.joinMs)
            ? firstEntry.joinMs
            : Math.min(
                  ...group.map((entry: any) => entry.joinMs).filter(Number.isFinite)
              );
        const endMs = Number.isFinite(firstEntry.leaveMs)
            ? firstEntry.leaveMs
            : Math.max(
                  ...group.map((entry: any) => entry.leaveMs).filter(Number.isFinite)
              );
        onOpenPreviousInstanceInfo?.({
            location,
            worldName,
