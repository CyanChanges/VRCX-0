...
describe('notification row helpers', () => {
    it('builds readable notification date, message, and group labels', () => {
        ...
        expect(getNotificationCreatedAt(notification)).toBe(
    ...
    it('keeps remote decline actions only for actionable notifications', () => {
        ...
        expect(canDeclineNotification({ type: 'message', link: '' })).toBe(
            false
        );
        expect(
            canDeclineNotification({ type: 'group.announcement', link: '' })
        ).toBe(false);
        expect(
            canDeclineNotification({ type: 'invite', link: 'economy.alert' })
        ).toBe(false);
    });

    it('normalizes response labels, emoji images, cached instances, and invite messages', () => {
        expect(getResponseLabel({ text: 'Accept', type: 'accept' })).toBe(
            'Accept'
        );
    ...
    it('shows invite cooldown labels from the remaining lockout time', () => {
        ...
        expect(getInviteCooldownLabel('2026-01-01T12:00:00Z', nowMs)).toBe(
            '30m'
        );
